...
const ResetPassword = () => {
    ...
    const onSubmit = async (data: FieldValues) => {
        ...
        await tryCatch(
            ...
            async () => {
                await deleteCookies([authKey, refreshKey]), router.push("/login");
            }
        );
    };
    ...
    return (
        <Box
            ...
            <Box boxShadow={1} maxWidth={600} width={"100%"} textAlign={"center"} p={{ xs: 2, lg: 5 }}>
                ...
                <N_Form onSubmit={onSubmit} resolver={zodResolver(resePasswordSchema)}>
                    <Stack direction={{ xs: "column", md: "row" }} spacing={{ xs: 2, md: 2 }} mt={2}>
                        <N_Input name="newPassword" label="New Password" type="password" />
                        <N_Input name="confirmPassword" label="Confirm New Password" type="password" />
                    </Stack>
